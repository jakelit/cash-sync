...
- BaseImporter unit tests: 27 test cases (UT001-UT002, UT051-UT060, UT075, UT005-UT016)
...
| UT059   | Unit           | `transform_transactions()`    | Row processing with invalid data                         | Row with invalid data (unparseable date, amount)                               | Row processed with default values (empty date, 0.0 amount), warnings logged                        | High     | Invalid data handling      | test_base_importer_unit.py                 |
| UT060   | Unit           | `transform_transactions()`    | Mixed valid and invalid rows                             | DataFrame with valid rows and rows with invalid data                           | All rows processed: valid rows normally, invalid data rows with default values                       | High     | Mixed data handling        | test_base_importer_unit.py                 |
...
| UT075   | Unit           | `transform_transactions()`    | Automatic column mapping with transformation override    | CSV with Amount column in raw format, Excel has Amount column                  | CSV Amount automatically mapped but transformed by BaseImporter logic (not raw CSV value)           | High     | Automatic mapping priority | test_base_importer_unit.py                 |
